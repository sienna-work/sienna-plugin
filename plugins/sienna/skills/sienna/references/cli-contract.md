...
- `meta get`, `google query`, and a completed natural-language `request` intentionally return upstream JSON without the Sienna success envelope.
- `request --json` returns a typed envelope only when it needs user input; its `data` includes `status`, `question`, `answer_contract`, and the exact `continue_command`.
...
sienna request "최근 7일 메타 광고 성과를 보여줘" --json
```
...
- Natural-language `needs_input`: ask the user the returned question, then run `sienna continue-request "<exact answer>" --json` in a new invocation. Do not invent the answer.
